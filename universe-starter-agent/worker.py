...
## Imports for catastrophe/blocking wrappers
...
def run(args, server):
    ...
    catastrophe_dir = os.path.join(args.log_dir, "catastrophes", "w{}".format(args.task))
    ...
    if args.online:
        if max_episodes is not None:
            print("WARNING: Setting max_episodes to None because we are in online labelling mode")
            max_episodes = None
        env = frame.HumanLabelWrapper(args.env_id, env, **hparams)

    env = frame.FrameSaverWrapper(env,
                                  os.path.join(args.log_dir, "episodes", "w{}".format(args.task)),
                                  max_episodes)
...
"""
    ...
    parser.add_argument(
        '-v', '--verbose', action='count', dest='verbosity', default=0, help='Set verbosity.')
